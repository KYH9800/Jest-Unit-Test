...
//! 성공 케이스만 검증한 Repository의 단위 테스트랑 다르게,
//! Service에서는 에러를 고의로 발생시켜 정상적으로 에러가 발생하는지 또한 검증할 예정입니다.

//? 이걸로도 test 대상을 만들 수 있다. (googling: 테스트 대상, 테스트 더블)
//* [mock / 지금 쓴거]
//* fake[fake postService를 하나 만든다, postService랑 동작이 비슷한 녀석을 하나 만든다]
//* stub
//* spy
...
describe('Layered Architecture Pattern Posts Service Unit Test', () => {
  // 1. 각 test가 실행되기 전에 실행됩니다.
  beforeEach(() => {
    jest.resetAllMocks(); // 모든 Mock을 초기화합니다.
  });

  //* Posts Service findAllPost Method
  test('Posts Service findAllPost Method', async () => {
    ...
    // 2. Repository의 findAllPost Method를 Mocking하고,
    // findAllPostReturnValue를 Return 값으로 변경합니다.
    mockPostsRepository.findAllPost = jest.fn(() => {
      return findAllPostReturnValue;
    });

    // 3. PostService의 findAllPost Method를 실행합니다. / 하면은...
    const allPost = await postService.findAllPost();

    // 4. allPost의 값이 postRepository의 findAllPost Method 결과값을
    // 내림차순으로 정렬한 것이 맞는지 검증합니다.
    ...
    // 5. PostRepository의 findAllPost Method는 1번 호출되었는지 검증합니다.
    expect(mockPostsRepository.findAllPost).toHaveBeenCalledTimes(1);
  });

  //* Posts Service deletePost Method By Success
  test('Posts Service deletePost Method By Success', async () => {
    ...
    const findPostByIdReturnValue = {
      ...
      content: 'Content_1',
      password: '1234',
      createAt: new Date('06 October 2011 15:50 UTC'),
      updatedAt: new Date('06 October 2011 15:50 UTC'),
    };
    ...
    */
    ...
    //! toHaveBeenCalledWith: findPostByIdReturnValue.postId 넘겨받아 실행이 되었다.
  ...
  //* Posts Service deletePost Method By Not Found Post Error
